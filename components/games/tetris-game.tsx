...
import { Play, Pause, RotateCw, ArrowDown, ArrowLeft, ArrowRight, Home } from "lucide-react"
      ...
      <div className="min-h-screen bg-black text-cyan-400 p-4 font-mono">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <h1 className="text-4xl font-bold text-center text-cyan-400 animate-pulse">TETRIS</h1>
            <div className="w-20" />
          </div>

          <div className="text-center mb-6">
            <p className="text-cyan-300 text-sm">Arrow keys: move • Up/Space: rotate • Enter: hard drop • P: pause</p>
          </div>

          <div className="flex flex-col lg:flex-row gap-6 items-start justify-center">
            {/* Game Board */}
            <Card className="bg-gray-900 border-cyan-400 p-4">
              <div className="border-2 border-cyan-400 bg-black p-2">{renderBoard()}</div>
            </Card>

            {/* Game Info */}
            <div className="flex flex-col gap-4">
              {/* Score */}
              <Card className="bg-gray-900 border-cyan-400 p-4 min-w-48">
                <div className="space-y-2">
                  <div className="text-xl font-bold text-cyan-400 mb-4">Stats</div>
                  ...
                  <div className="flex justify-between">
                    <span>Lines:</span>
                    <span className="text-cyan-300">{lines}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Level:</span>
              ...
              {/* Next Piece */}
              <Card className="bg-gray-900 border-cyan-400 p-4">
                <div className="text-xl font-bold text-cyan-400 mb-4">Next</div>
              ...
              <Card className="bg-gray-900 border-cyan-400 p-4">
                ...
                <div className="space-y-3">
                  {!isPlaying ? (
                      <Button onClick={startGame} className="w-full bg-cyan-600 hover:bg-cyan-700">
                        <Play className="w-4 h-4 mr-2" />
                        {gameOver ? "New Game" : "Start Game"}
                      </Button>
                  ) : (
                      <Button onClick={() => setIsPaused(!isPaused)} className="w-full bg-yellow-600 hover:bg-yellow-700">
                        {isPaused ? (
                            <>
                              <Play className="w-4 h-4 mr-2" />
                              Resume
                  ...
                  {gameOver && <div className="text-center text-red-400 font-semibold">GAME OVER!</div>}

                  {isPaused && isPlaying && <div className="text-center text-yellow-400">PAUSED</div>}
                </div>
              </Card>

              {/* Mobile Controls */}
              <Card className="bg-gray-900 border-cyan-400 p-4 lg:hidden">
                <div className="text-xl font-bold text-cyan-400 mb-4">Touch Controls</div>
                <div className="grid grid-cols-3 gap-2">
                  <Button
                      variant="outline"
                      size="sm"
                      onTouchStart={() => movePiece("left")}
                      className="aspect-square border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-black bg-transparent"
                  >
                  ...
                  <Button
                      ...
                      className="aspect-square text-xs border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-black bg-transparent"
                  >
                    Drop
                  </Button>
                </div>
              ...
              <Card className="bg-gray-900 border-cyan-400 p-4">
                ...
                <div className="text-sm space-y-2">
                  ...
                  <p>• Enter for instant hard drop</p>
